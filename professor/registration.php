...
if (count($errors)>0){
    ...
    require_once "database.php";
    $sql = "INSERT INTO professor (full_name, email, disci, tele, password) VALUES ( ?, ?, ?, ?, ? )";
    $stmt = mysqli_stmt_init($conn);
    $prepareStmt = mysqli_stmt_prepare($stmt,$sql);
    if ($prepareStmt) {
      mysqli_stmt_bind_param($stmt,"sss",$fullName,$email, $passwordhash, $disci, $tele);
...
?>
  ...
  <form action="registration.php" method="post">
    ...
    <div class="form-group">
      <input type="test" class="form-control" name="disci" placeholder="Disciplina que leciona">
    </div>
    <div class="form-group">
      <input type="test" class="form-control" name="tele" placeholder="Telefone">
    </div>
...
<div>
<div><p>Ja se registou?<a href="login.php">Login se aqui</a></p></div>
</div>
</div>
</body>
</html>